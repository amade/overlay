...
LICENSE="LGPL-2+"
SLOT="3"
IUSE="accessibility aqua broadway cloudprint colord cups dbus examples +introspection test vim-syntax wayland X xinerama"
REQUIRED_USE="
	|| ( aqua wayland X )
	xinerama? ( X )
"
...
src_prepare() {
	...
	if ! use accessibility; then
		epatch "${FILESDIR}/${PN}-3.16.5-atk-choice.patch"
	fi
	if ! use dbus; then
		epatch "${FILESDIR}/${PN}-3.14.13-no-dbus.patch"
		epatch "${FILESDIR}/${PN}-3.16.5-no-dbus-part2.patch"
		epatch "${FILESDIR}/${PN}-3.16.5-gtk-Makefile.am.patch"
	fi
